...
async function handler(req, res) {
    ...
    try {
        ...
        if (!user || !user.isAdmin) return res.status(403).json({
            message: "Seul l'administrateur peut supprimer"
        });
        ...
        const tryUnlink = async (url)=>{
            ...
            if (url.startsWith("/uploads/")) {
                const fpath = path__WEBPACK_IMPORTED_MODULE_0___default().join(process.cwd(), "public", url.replace("/uploads/", "uploads/"));
                try {
                    await fs__WEBPACK_IMPORTED_MODULE_1__.promises.unlink(fpath);
                } catch (_) {}
            }
        };
        ...
        res.status(500).json({
            message: "\xc9chec de la suppression"
        });
    }
}
